...
/* The sum of twelve uniform random numbers -6 has the same mean
 * and variance as a standard normal variable. */
static inline double samplePseudoNorm() {
    double sum=-6;
    for (int i=0; i<12; i++) {
...
void write_test_file(const char *h5fname, const char *simname) {
    ...
    for (int i=0; i<M; i++) {
        for (int j=0; j<N; j++) {
            samples[j + N*i] = samplePseudoNorm() * i;
        }
    }
